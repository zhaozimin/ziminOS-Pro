...
function renderTable(app, el, sourcePath, headers, rows, grow) {
  ...
  headers.forEach((header2, index) => {
    headRow.createEl("th", {
      cls: classOf(index),
      text: index === 0 && rows.length ? `${header2} (${rows.length})` : header2
    });
  });
...
function normalizeSettings(input) {
  ...
  const filePathScope = isFilePathScope(stored.filePathScope) ? stored.filePathScope : DEFAULT_SETTINGS.filePathScope;
  ...
  const eaglePort = isEaglePort(stored.eaglePort) ? stored.eaglePort : DEFAULT_SETTINGS.eaglePort;
  return {
...
function isFilePathScope(value) {
...
function isEaglePort(value) {
  return typeof value === "number" && Number.isInteger(value) && value >= EAGLE_PORT_RANGE.min && value <= EAGLE_PORT_RANGE.max;
}
...
async function openSnippetFile(app, name) {
  const shell = requireShell();
...
function resolveShell() {
  ...
  try {
    ...
    return typeof (shell == null ? void 0 : shell.openPath) === "function" && typeof (shell == null ? void 0 : shell.showItemInFolder) === "function" ? shell : null;
  } catch (e) {
    return null;
  }
}
function absolutePath(app, relative) {
...
var AppearanceSwitch = class {
  ...
  render(panel, snippets) {
    const header2 = panel.createDiv({ cls: "ziminos-appearance-header" });
    header2.createSpan({ text: TEXTS.title });
    header2.createSpan({
   ...
   * 说清它去哪儿的成本只付一遍；每行那个要出现十三次，十三个「打开这个 CSS 文件」
...
async function excerptCard(ctx) {
  ...
  const { app } = ctx;
  try {
    const view = app.workspace.getActiveViewOfType(import_obsidian10.MarkdownView);
    const mocFile = (_a = view == null ? void 0 : view.file) != null ? _a : null;
    if (!view || !mocFile || !isBookMoc(ctx, mocFile)) {
      new import_obsidian10.Notice(MESSAGES2.notBookMoc);
      return;
    }
    const selection = view.editor.getSelection().trim();
    if (!selection) {
      new import_obsidian10.Notice(view.getMode() === "preview" ? MESSAGES2.readingMode : MESSAGES2.noSelection);
      return;
    }
    ...
    if (nameInput === null || !nameInput.trim()) {
      new import_obsidian10.Notice(MESSAGES2.nameMissing);
      return;
    }
    const cardName = nameInput.trim();
    if (/[\\/]/.test(cardName)) {
      new import_obsidian10.Notice(MESSAGES2.nameIllegal);
      return;
    }
    ...
    const cardPath = (0, import_obsidian10.normalizePath)(
      folderPath ? `${folderPath}/${cardName}.md` : `${cardName}.md`
    );
    if (app.vault.getAbstractFileByPath(cardPath)) {
      new import_obsidian10.Notice(MESSAGES2.existsPrefix + cardPath);
      return;
    }
    ...
    new import_obsidian10.Notice(MESSAGES2.createdPrefix + cardName);
  } catch (error) {
    const message2 = error instanceof Error ? error.message : String(error);
    new import_obsidian10.Notice(MESSAGES2.failedPrefix + message2);
  }
}
...
async function runImport(ctx) {
  ...
  try {
    ...
    if (!target) {
      if (allBookMocs(ctx).length) new import_obsidian11.Notice(MESSAGES3.cancelled);
      return;
    }
    ...
    if (!raw.trim()) {
      new import_obsidian11.Notice(MESSAGES3.pasteEmpty);
      return;
    }
    const parsed = parseHighlightExport(raw);
    if (!parsed) {
      new import_obsidian11.Notice(MESSAGES3.unrecognized, 1e4);
      return;
    }
    ...
    if (!book.highlights.length) {
      new import_obsidian11.Notice(MESSAGES3.emptyBook);
      return;
    }
    ...
    new import_obsidian11.Notice(describeImported(outcome, targetName));
  } catch (error) {
    const message2 = error instanceof Error ? error.message : String(error);
    new import_obsidian11.Notice(MESSAGES3.failedPrefix + message2);
  }
}
...
async function resolveTargetBook(ctx) {
  ...
  if (!books.length) {
    new import_obsidian11.Notice(MESSAGES3.noBooks);
    return null;
  }
...
var ImportConfirmModal = class extends import_obsidian11.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian11.ButtonComponent(buttonBar).setButtonText("\u53D6\u6D88").onClick(() => this.settle(false));
    new import_obsidian11.ButtonComponent(buttonBar).setButtonText("\u786E\u8BA4\u5BFC\u5165").setCta().onClick(() => this.settle(true));
  }
  onClose() {
    this.settle(false);
    this.contentEl.empty();
...
var doubanFetcher = async (url) => {
  const response = await (0, import_obsidian12.requestUrl)({ url, method: "GET", headers: BROWSER_HEADERS });
...
function wereadAvailable(ctx) {
  return import_obsidian13.Platform.isDesktopApp && !!ctx.settings.wereadCookie.trim();
}
...
var cachedNodeTools;
function nodeTools() {
  if (!import_obsidian14.Platform.isDesktopApp) return null;
...
function appleBooksAvailable() {
  if (!import_obsidian14.Platform.isDesktopApp || process.platform !== "darwin") return false;
...
function nodeTools2() {
  if (!import_obsidian15.Platform.isDesktopApp) return null;
...
function runBookCommand(task, failurePrefix) {
  void task().catch((error) => {
    new import_obsidian16.Notice(failurePrefix + (describe2(error) || "\u672A\u77E5\u9519\u8BEF"), 1e4);
  });
}
async function readBook(ctx, create) {
  ...
  if (input === null || !input.trim()) {
    new import_obsidian16.Notice(MESSAGES4.nameMissing);
    return;
  }
  const searching = new import_obsidian16.Notice(MESSAGES4.searching, 0);
  let candidates;
  try {
    candidates = await searchBooks(doubanFetcher, input.trim());
  } catch (error) {
    searching.hide();
    new import_obsidian16.Notice(describe2(error) || MESSAGES4.searchFailed, 1e4);
    return;
  }
  searching.hide();
  if (!candidates.length) {
    new import_obsidian16.Notice(MESSAGES4.noResult, 8e3);
    return;
  }
  ...
  try {
    ...
    new import_obsidian16.Notice(MESSAGES4.detailFailed + describe2(error), 8e3);
    return;
  }
...
async function pullHighlights(ctx, moc, names, author) {
  ...
  if (!labels.length) {
    new import_obsidian16.Notice(MESSAGES4.noSource, 1e4);
    return;
  }
  const title = (_a = names[0]) != null ? _a : "";
  const pulling = new import_obsidian16.Notice(MESSAGES4.pulling, 0);
  let hits;
  try {
...
async function pickBook(ctx) {
  const books = allBookMocs(ctx);
  if (!books.length) {
    new import_obsidian16.Notice("\u8FD8\u6CA1\u6709\u4EFB\u4F55\u8BFB\u4E66\u7B14\u8BB0\u3002\u5148\u8FD0\u884C\u300C\u8BFB\u4E00\u672C\u4E66\u300D\u3002");
    return null;
  }
  return new ChoiceModal(ctx.app, {
    title: "\u540C\u6B65\u54EA\u672C\u4E66\u7684\u5212\u7EBF\uFF1F",
...
var CursorMemory = class {
  constructor(ctx) {
    ...
    plugin.registerEvent(
      app.vault.on("rename", (file, oldPath) => {
        ...
        if (mark && file instanceof import_obsidian17.TFile) this.marks.set(file.path, mark);
        if (((_a = this.tracked) == null ? void 0 : _a.path) === oldPath && file instanceof import_obsidian17.TFile) {
  ...
  adopt(restore) {
    var _a;
    const view = this.ctx.app.workspace.getActiveViewOfType(import_obsidian17.MarkdownView);
...
var EAGLE_URI_SOURCE = "ziminos-eagle:\\/\\/v1\\/[A-Za-z0-9_-]{1,128}\\/[A-Za-z0-9_-]{1,128}";
...
function normalizeEaglePort(value) {
  return typeof value === "number" && Number.isInteger(value) && value >= EAGLE_PORT_RANGE.min && value <= EAGLE_PORT_RANGE.max ? value : EAGLE_DEFAULT_PORT;
}
function buildEagleUri(reference) {
...
function eagleReferenceAtText(value, offset) {
...
function singleEagleReferenceInText(value) {
  const matches = eagleUriMatches(value);
  return matches.length === 1 ? matches[0].reference : null;
}
function buildEagleMarkdown(reference, fileName, mimeType) {
...
function eagleUriMatches(value) {
  var _a, _b;
  const matches = [];
  ...
  while ((result = pattern.exec(value)) !== null) {
    ...
    const before = (_a = value[from - 1]) != null ? _a : "";
    const after = (_b = value[to]) != null ? _b : "";
...
function interactiveRange(value, uriFrom, uriTo) {
...
function previousUnescaped(value, needle, before) {
...
var EagleBridgeClient = class {
  ...
  async request(param, authenticated = true) {
    var _a;
    const headers = { ...(_a = param.headers) != null ? _a : {} };
    if (authenticated) {
  ...
  readToken() {
    var _a, _b;
    return (_b = (_a = this.ctx.app.secretStorage.getSecret(EAGLE_TOKEN_SECRET_ID)) == null ? void 0 : _a.trim()) != null ? _b : "";
  }
  writeToken(token) {
...
function header(headers, name) {
  ...
  return (_a = found == null ? void 0 : found[1]) != null ? _a : "";
}
...
// src/modules/eagle/editor.ts
var import_state = require("@codemirror/state");
var import_view = require("@codemirror/view");
var import_obsidian20 = require("obsidian");
function createEagleEditorExtension(client) {
  return import_state.Prec.highest(import_view.EditorView.domEventHandlers({
    click: (event, view) => {
      var _a;
      if (event.button !== 0 || !event.metaKey && !event.ctrlKey) return false;
      const reference = (_a = eagleReferenceFromTarget(event.target)) != null ? _a : referenceFromPosition(event, view);
...
function referenceFromPosition(event, view) {
...
function eagleReferenceFromTarget(target) {
  ...
  for (let depth = 0; element && depth < 4; depth += 1, element = element.parentElement) {
    ...
    const text3 = controlText(element);
    if (text3.length <= 512) {
      const reference = singleEagleReferenceInText(text3);
...
function registerEagleRenderer(ctx, client) {
  if (!isSupportedEagleDesktop()) return () => void 0;
  const observers = /* @__PURE__ */ new Map();
  const objectUrls = /* @__PURE__ */ new Set();
  const imageUrls = /* @__PURE__ */ new Map();
  let loadGeneration = 0;
  let disposed = false;
  const objectUrl = async (reference) => {
  ...
  const release = (root) => {
    if (root.tagName === "IMG") releaseImage(root);
    for (const image of root.querySelectorAll("img[data-ziminos-eagle-uri]")) releaseImage(image);
  };
  const observe = (doc) => {
    var _a, _b;
    if (observers.has(doc) || !doc.body) return;
    const Observer = (_b = (_a = doc.defaultView) == null ? void 0 : _a.MutationObserver) != null ? _b : MutationObserver;
  ...
  const openAt = (event) => {
    var _a, _b;
    if (((_b = (_a = event.target) == null ? void 0 : _a.closest) == null ? void 0 : _b.call(_a, ".cm-editor")) && !event.metaKey && !event.ctrlKey) return;
  ...
  const unbindDocument = (doc) => {
    var _a;
    release(doc);
    (_a = observers.get(doc)) == null ? void 0 : _a.disconnect();
  ...
  return () => {
    for (const doc of observers.keys()) {
      for (const element of doc.querySelectorAll('img[data-ziminos-eagle-state="error"]')) {
...
async function materialize(file, name) {
  ...
  if (sourcePath) return { path: sourcePath, cleanup: async () => void 0 };
  const tools = nodeTools3();
  ...
  try {
    ...
    await tools.fs.rmdir(dir).catch(() => void 0);
    throw error;
  }
  return {
    path,
    cleanup: async () => {
      await tools.fs.unlink(path).catch(() => void 0);
      await tools.fs.rmdir(dir).catch(() => void 0);
    }
  };
}
function pathForFile(file) {
  ...
  try {
    const electron = require("electron");
    return (_d = (_c = (_b = (_a = electron.webUtils) == null ? void 0 : _a.getPathForFile) == null ? void 0 : _b.call(_a, file)) == null ? void 0 : _c.trim()) != null ? _d : "";
  } catch (e) {
    return "";
  }
}
function nodeTools3() {
  ...
  if (cachedNodeTools3 !== void 0) return cachedNodeTools3;
  try {
    cachedNodeTools3 = {
    ...
    cachedNodeTools3 = null;
  }
  return cachedNodeTools3;
}
...
function attachmentName(file) {
  ...
  return `\u9644\u4EF6.${(_a = extensions[file.type.toLowerCase()]) != null ? _a : "bin"}`;
}
function uniqueId() {
  var _a, _b, _c;
  return (_c = (_b = (_a = globalThis.crypto) == null ? void 0 : _a.randomUUID) == null ? void 0 : _b.call(_a)) != null ? _c : `${Date.now()}-${Math.random().toString(36).slice(2)}`;
}
...
function registerEagleBridge(ctx, resolveRoute) {
  ...
  return {
    pairEagle: async () => {
      ...
      const code = (_a = answer == null ? void 0 : answer.trim()) != null ? _a : "";
      if (!code) return false;
      if (!/^\d{6}$/.test(code)) {
        new import_obsidian23.Notice("\u914D\u5BF9\u7801\u5E94\u4E3A 6 \u4F4D\u6570\u5B57\u3002");
        return false;
      }
      try {
    ...
    disconnectEagle: async () => {
      ...
      await ctx.saveSettings();
      new import_obsidian23.Notice(revokedRemotely ? "\u5DF2\u65AD\u5F00 Eagle \u4F34\u4FA3\uFF1B\u7B14\u8BB0\u91CC\u5DF2\u6709\u7684\u8EAB\u4EFD\u94FE\u63A5\u6CA1\u6709\u88AB\u6539\u52A8\u3002" : "\u5DF2\u6E05\u9664 Obsidian \u672C\u673A\u51ED\u636E\uFF1BEagle \u4F34\u4FA3\u6682\u65F6\u4E0D\u53EF\u8FBE\uFF0C\u8BF7\u5728\u4F34\u4FA3\u7A97\u53E3\u624B\u52A8\u79FB\u9664\u8FD9\u672C\u5E93\u7684\u65E7\u6388\u6743\u3002", 1e4);
    },
    describeEagleStatus: async () => {
      if (!isSupportedEagleDesktop()) return "\u672A\u542F\u7528 \xB7 \u53EA\u652F\u6301 macOS / Windows \u684C\u9762\u7AEF";
      if (!client.hasToken()) return "\u672A\u914D\u5BF9 \xB7 \u5148\u5728 Eagle \u4E2D\u6253\u5F00\u4F34\u4FA3\u53D6\u5F97 6 \u4F4D\u7801";
      try {
        const status = await client.status();
    ...
    revealEaglePackage: async () => {
      ...
      if (!await ctx.app.vault.adapter.exists(relative)) {
        new import_obsidian23.Notice("Eagle \u4F34\u4FA3\u5B89\u88C5\u5305\u4E0D\u5728\u63D2\u4EF6\u76EE\u5F55\uFF0C\u8BF7\u91CD\u65B0\u5347\u7EA7 ziminOS\u3002");
        return;
      }
...
function walk(folder, recursive, tallies) {
  ...
  for (const child of folder.children) {
    ...
    if (child instanceof import_obsidian24.TFile && child.extension === "md") notes += 1;
    else others += 1;
  }
...
var FilePathBar = class {
  ...
  sync() {
    ...
    this.statusEl.setText((_a = this.vaultPath()) != null ? _a : TEXTS3.empty);
    (0, import_obsidian26.setTooltip)(
      this.statusEl,
      resolved ? TEXTS3.tooltipPrefix + resolved.text : TEXTS3.tooltipEmpty,
      { placement: "top" }
    );
  }
  /**
  ...
  resolve() {
    ...
    return full === null ? { text: relative, degraded: true } : { text: full, degraded: false };
  }
  /** 复制路径。状态栏那一块与命令走的是这同一段，因此两条路的结果与提示语一字不差 */
  async copy() {
    const resolved = this.resolve();
    if (!resolved) {
      new import_obsidian26.Notice(TEXTS3.noFile);
      return;
    }
    try {
      await navigator.clipboard.writeText(resolved.text);
      new import_obsidian26.Notice((resolved.degraded ? TEXTS3.degraded : TEXTS3.copied) + resolved.text);
    } catch (error) {
      new import_obsidian26.Notice(TEXTS3.failed + (error instanceof Error ? error.message : String(error)));
    }
  }
};
...
var RecentFilesStore = class {
  ...
  list(sort, limit) {
    ...
    for (const entry of this.entries) {
      ...
      if (file instanceof import_obsidian27.TFile) files.push(file);
    }
...
var RecentFilesView = class extends import_obsidian27.ItemView {
...
function captureMarkdownScroll(app, path) {
  const marks = [];
  app.workspace.iterateAllLeaves((leaf) => {
    var _a;
    if (!(leaf.view instanceof import_obsidian28.MarkdownView) || ((_a = leaf.view.file) == null ? void 0 : _a.path) !== path) return;
...
function restoreMarkdownScroll(marks) {
  var _a;
  for (const mark of marks) {
    if (((_a = mark.view.file) == null ? void 0 : _a.path) !== mark.path || mark.view.getMode() !== mark.mode) continue;
...
function registerFormatter(ctx) {
  ...
  ctx.commands.register(FORMAT_COMMAND, () => {
    ...
    if (!file || file.extension !== "md") {
      new import_obsidian29.Notice(TEXTS5.noFile);
      return;
    }
    if (ctx.settings.formatRules.length === 0) {
      new import_obsidian29.Notice(TEXTS5.noRules);
      return;
    }
    void formatFile(file).then((changed) => {
      dirtyWhileOpen.delete(file.path);
      new import_obsidian29.Notice(changed ? TEXTS5.formatted : TEXTS5.unchanged);
    }).catch(() => {
      new import_obsidian29.Notice(TEXTS5.failed);
    });
  });
}
...
function run(label, opener) {
  if (!opener) {
    new import_obsidian30.Notice(label + UNAVAILABLE_SUFFIX);
    return;
  }
  opener();
}
...
var HolidayService = class {
  ...
  async fetchDataset(year) {
    for (const source of SOURCES) {
      try {
        const response = await (0, import_obsidian31.requestUrl)({ url: source(year), throw: false });
...
var ZiminosCalendarView = class extends import_obsidian32.ItemView {
   ...
   */
  renderHeader(parent) {
    const header2 = parent.createDiv({ cls: "ziminos-calendar-header" });
    const firstRow = header2.createDiv({ cls: "ziminos-calendar-header-row" });
    ...
    const secondRow = header2.createDiv({ cls: "ziminos-calendar-header-row" });
...
async function pickPerson(ctx, title) {
  ...
  if (!candidates.length) {
    new import_obsidian33.Notice("\u8FD8\u6CA1\u6709\u4EFB\u4F55\u4EBA\u8109\u6216\u5BA2\u6237\u6863\u6848\u3002\u5148\u8FD0\u884C\u300C\u65B0\u5EFA\u4EBA\u8109\u300D\u5EFA\u4E00\u4E2A\uFF0C\u518D\u6765\u5173\u8054\u3002");
    return null;
  }
  return new ChoiceModal(ctx.app, {
    title,
  ...
  render: async (view) => {
    ...
    for (const client of view.index.notesOfType(NOTE_TYPES.client)) {
      ...
      rows.push({
        ...
        created: (_a = dayText(view.index.fieldOf(client, FIELDS.created))) != null ? _a : dayOfMillis(client.stat.ctime)
      });
    }
    if (!rows.length) {
    ...
    rows.sort(
      (left, right) => Number(right.pendingCount > 0) - Number(left.pendingCount > 0) || right.amount - left.amount || left.created.localeCompare(right.created)
    );
...
async function setupClients(ctx, applySeed2) {
  try {
    ...
    new import_obsidian34.Notice(MESSAGES5.setupDone);
...
async function createClient(ctx) {
  try {
    ...
    if (!name) {
      new import_obsidian34.Notice(MESSAGES5.cancelled);
      return;
    }
    if (ILLEGAL_NAME.test(name)) {
      new import_obsidian34.Notice(MESSAGES5.illegalName);
      return;
    }
    ...
    if (existing instanceof import_obsidian34.TFile) {
      new import_obsidian34.Notice(MESSAGES5.existsPrefix + name);
    ...
    new import_obsidian34.Notice(MESSAGES5.createdPrefix + name);
...
async function addPayment(ctx) {
  try {
    ...
    if (!clients.length) {
      new import_obsidian34.Notice(MESSAGES5.noClients);
      return;
    }
    ...
    new import_obsidian34.Notice(`${MESSAGES5.paidPrefix}${client.basename} \xB7 ${product} \xB7 ${amount}`);
  } catch (error) {
    notifyFailure(error);
  }
}
async function recordReceipt(ctx) {
  try {
    ...
    if (!projects.length) {
      new import_obsidian34.Notice(MESSAGES5.noProjects);
      return;
    }
    ...
    new import_obsidian34.Notice(`${MESSAGES5.receiptPrefix}${project.basename} \xB7 ${amount}`);
  } catch (error) {
    notifyFailure(error);
  }
}
...
async function askAmount(ctx) {
  ...
  if (!Number.isFinite(amount) || amount <= 0) {
    new import_obsidian34.Notice(MESSAGES5.amountInvalid);
    return null;
  }
  return amount;
}
...
function notifyFailure(error) {
  const message2 = error instanceof Error ? error.message : String(error);
  new import_obsidian34.Notice(MESSAGES5.failedPrefix + message2);
}
...
async function createContact(ctx) {
  try {
    ...
    const name = (answer != null ? answer : "").trim();
    if (!name) {
      new import_obsidian35.Notice(MESSAGES6.cancelled);
      return;
    }
    if (ILLEGAL_NAME2.test(name)) {
      new import_obsidian35.Notice(MESSAGES6.illegalName);
      return;
    }
    ...
    const path = `${folder}/${name}.md`;
    const existing = ctx.app.vault.getAbstractFileByPath(path);
    if (existing instanceof import_obsidian35.TFile) {
      new import_obsidian35.Notice(MESSAGES6.existsPrefix + name);
      await ctx.app.workspace.getLeaf(false).openFile(existing);
      return;
    }
    const { stamp, uid } = nowStampAndUid(ctx.settings.dateTimeFormat);
    ...
    ctx.guard.mark(path);
    const file = await ctx.app.vault.create(path, content);
    new import_obsidian35.Notice(MESSAGES6.donePrefix + name);
    await ctx.app.workspace.getLeaf(false).openFile(file);
  } catch (error) {
    const message2 = error instanceof Error ? error.message : String(error);
    new import_obsidian35.Notice(MESSAGES6.failedPrefix + message2);
  }
}
...
async function recordFavor(ctx, openDaily) {
  try {
    ...
    if (!candidates.length) {
      new import_obsidian36.Notice(MESSAGES7.noContacts);
      return;
    }
    ...
    if (!person) {
      new import_obsidian36.Notice(MESSAGES7.cancelled);
      return;
    }
    ...
    if (!diary) {
      new import_obsidian36.Notice(MESSAGES7.noDiary);
      return;
    }
    ...
    new import_obsidian36.Notice(MESSAGES7.donePrefix + line);
  } catch (error) {
    const message2 = error instanceof Error ? error.message : String(error);
    new import_obsidian36.Notice(MESSAGES7.failedPrefix + message2);
  }
}
...
async function captureInspiration(ctx) {
  try {
    ...
    if (!inspiration) {
      new import_obsidian37.Notice("\u672A\u8F93\u5165\u5185\u5BB9\uFF0C\u64CD\u4F5C\u5DF2\u53D6\u6D88\u3002");
      return;
    }
    ...
    if (targetEntry instanceof import_obsidian37.TFolder) {
    ...
    if (!targetEntry) {
      ...
      if (!(targetEntry instanceof import_obsidian37.TFile) || targetEntry.extension.toLowerCase() !== "md") {
    ...
    new import_obsidian37.Notice(`\u5DF2\u8BB0\u5F55\u7075\u611F\uFF1A${inspiration}`);
  } catch (error) {
    const message2 = error instanceof Error ? error.message : String(error);
    new import_obsidian37.Notice(`\u8BB0\u5F55\u7075\u611F\u5931\u8D25\uFF1A${message2}`);
  }
}
function resolveInspirationTarget(ctx) {
  ...
  const path = (0, import_obsidian37.normalizePath)(folder ? `${folder}/${fileName}` : fileName);
...
function mocPathOf(folderPath, containerName) {
  return (0, import_obsidian38.normalizePath)(`${folderPath}/${mocBasenameOf(containerName)}.md`);
}
function legacyMocPathOf(folderPath, containerName) {
  return (0, import_obsidian38.normalizePath)(`${folderPath}/${containerName}.md`);
}
...
function getCardContext(app, filePath, roots) {
  const normalizedFilePath = (0, import_obsidian39.normalizePath)(filePath);
...
async function initCard(ctx, file, opts) {
  ...
  try {
    ...
    const message2 = error instanceof Error ? error.message : String(error);
    new import_obsidian39.Notice(`\u5361\u7247\u7B14\u8BB0\u521D\u59CB\u5316\u5931\u8D25\uFF1A${message2}`);
    throw error;
  }
}
  ...
  ctx.app.workspace.onLayoutReady(() => {
    ctx.plugin.registerEvent(
      ctx.app.vault.on("create", (file) => {
        ...
        if (!(file instanceof import_obsidian39.TFile) || file.extension !== "md") return;
...
async function createContainer(ctx, kind, preset, pickPerson2) {
  ...
  try {
    ...
    if (nameInput === null || !nameInput.trim()) {
      new import_obsidian40.Notice(`\u672A\u8F93\u5165${kind.label}\u540D\u79F0\uFF0C\u64CD\u4F5C\u5DF2\u53D6\u6D88\u3002`);
      return null;
    }
    const containerName = nameInput.trim();
    if (/[\\/]/.test(containerName)) {
      new import_obsidian40.Notice(`${kind.label}\u540D\u79F0\u4E0D\u80FD\u5305\u542B\u659C\u6760\u6216\u53CD\u659C\u6760\u3002`);
      return null;
    }
    let relation;
    if (kind.asksOwnership && !preset) {
      ...
      if (!ownership) {
        new import_obsidian40.Notice(`\u672A\u9009\u62E9${kind.label}\u5F52\u5C5E\uFF0C\u64CD\u4F5C\u5DF2\u53D6\u6D88\u3002`);
        return null;
      }
      if (ownership.link && pickPerson2) {
        ...
        if (required && !person) {
          new import_obsidian40.Notice("\u672A\u9009\u62E9\u5BA2\u6237\uFF0C\u64CD\u4F5C\u5DF2\u53D6\u6D88\u3002");
          return null;
        }
        if (person) {
          relation = {
    ...
    if (descriptionInput === null) {
      new import_obsidian40.Notice(`\u5DF2\u53D6\u6D88\u8F93\u5165${kind.label}\u6982\u8FF0\uFF0C\u64CD\u4F5C\u5DF2\u53D6\u6D88\u3002`);
      return null;
    }
    ...
    if (existingMocFile) {
      new import_obsidian40.Notice(`${kind.label} MOC \u7B14\u8BB0\u5DF2\u7ECF\u5B58\u5728\uFF0C\u672A\u6267\u884C\u8986\u76D6\uFF1A${mocFilePath}`);
      return null;
    }
    ...
    new import_obsidian40.Notice(`${kind.label}\u5DF2\u521B\u5EFA\uFF1A${containerName}`);
    return mocFile;
  } catch (error) {
    const message2 = error instanceof Error ? error.message : String(error);
    new import_obsidian40.Notice(`\u521B\u5EFA${kind.label}\u5931\u8D25\uFF1A${message2}`);
    return null;
  }
}
...
async function recordExport(ctx, container) {
  ...
  try {
    ...
    new import_obsidian41.Notice(`\u5DF2\u8BB0\u8FDB\u51FA\u5E93\u5355\uFF1A\u4E0B\u6B21\u548C\u667A\u80FD\u4F53\u8BF4\u8BDD\u65F6\uFF0C\u300A${container.name}\u300B\u4F1A\u642C\u8FDB\u300A${layout.eternal}\u300B\u3002`);
  } catch (error) {
    new import_obsidian41.Notice(`\u51FA\u5E93\u5355\u6CA1\u8BB0\u6210\uFF08\u9879\u76EE\u5DF2\u7ECF\u6B63\u5E38\u5F52\u6863\uFF09\uFF1A${message(error)}`);
  }
}
async function readOrCreateManifest(ctx, eternalVaultName) {
  ...
  if (file instanceof import_obsidian41.TFile) return ctx.app.vault.read(file);
...
async function readNote(view, path) {
  ...
  return file instanceof import_obsidian42.TFile ? view.ctx.app.vault.read(file) : null;
}
...
var MigrationBatchError = class extends Error {
  constructor(operationError, rollbackFailures) {
    const operationMessage = errorMessage6(operationError);
    const detail = rollbackFailures.length ? `\u81EA\u52A8\u56DE\u6EDA\u672A\u80FD\u6062\u590D\uFF1A${rollbackFailures.join("\uFF1B")}` : "\u672C\u8F6E\u5DF2\u4FEE\u6539\u6587\u4EF6\u5DF2\u5168\u90E8\u56DE\u6EDA";
...
var CURRENT_MOC_BASE = mocBaseBlock();
var CURRENT_NAV_BASE = currentNavigationBase();
var MIGRATABLE_MOC_TYPES = /* @__PURE__ */ new Set([
  NOTE_TYPES.project,
  NOTE_TYPES.area,
  NOTE_TYPES.book
]);
...
var LEGACY_HARDCODED_MOC_SKELETON = [
  "```base",
  "filters:",
  "  and:",
  "    - file.path != this.file.path",
...
function planMocBaseUpgrade(content) {
  ...
  if (scan.blocks.length === 0) {
    const afterBlock2 = withDocumentLineEnding(CURRENT_MOC_BASE, content);
    return {
      status: "change",
      action: "\u8865\u4E0A\u7F3A\u5931 Base",
      content: appendBlock(content, afterBlock2),
      beforeBlock: null,
...
function isContainerMocIdentity(basename, parentName, type) {
...
function planNavigationBaseUpgrade(content) {
  const scan = scanBaseBlocks(content);
  if (scan.unclosed) {
  ...
  if (scan.blocks.length !== 1) {
    return {
      status: "conflict",
      reason: scan.blocks.length === 0 ? "\u5BFC\u822A\u9875\u6CA1\u6709 Base\uFF0C\u65E0\u6CD5\u786E\u8BA4\u5176\u7ED3\u6784" : `\u5BFC\u822A\u9875\u5B58\u5728 ${scan.blocks.length} \u4E2A Base\uFF0C\u65E0\u6CD5\u5224\u65AD\u8BE5\u66FF\u6362\u54EA\u4E00\u4E2A`
    };
  }
  const block = scan.blocks[0];
  const normalized = normalizeLineEndings(block.text);
...
async function applyMigrationBatch(items, applyItem, rollbackItem, describeItem) {
  const attempted = [];
  try {
    for (const item of items) {
      attempted.push(item);
    ...
    for (const item of [...attempted].reverse()) {
      try {
        ...
        rollbackFailures.push(`${describeItem(item)}\uFF08${errorMessage6(rollbackError)}\uFF09`);
      }
    }
    throw new MigrationBatchError(operationError, rollbackFailures);
  }
}
function currentNavigationBase() {
...
function isLegacyHardcodedMocBase(block) {
  var _a, _b, _c;
  const lines = block.split("\n");
  if (lines.length !== LEGACY_HARDCODED_MOC_SKELETON.split("\n").length) return false;
  if (!/^    name: (项目文件|读书卡片)$/.test((_a = lines[11]) != null ? _a : "")) return false;
  if (!isJsonStringExpression((_b = lines[14]) != null ? _b : "", "        - up == link(", ")")) return false;
  if (!isJsonStringExpression((_c = lines[15]) != null ? _c : "", "        - file.folder == ", "")) return false;
...
function isJsonStringExpression(line, prefix, suffix) {
...
function scanBaseBlocks(content) {
...
function replaceBlock(content, block, replacement) {
...
function appendBlock(content, block) {
  ...
  const lineEnding = lineEndingOf(content);
  const trailingBreaks = (_d = (_c = (_b = (_a = content.match(/(?:(?:\r\n|\r|\n))+$/)) == null ? void 0 : _a[0]) == null ? void 0 : _b.match(/\r\n|\r|\n/g)) == null ? void 0 : _c.length) != null ? _d : 0;
  const gap = lineEnding.repeat(Math.max(0, 2 - trailingBreaks));
  return `${content}${gap}${block}${lineEnding}`;
}
function withDocumentLineEnding(block, content) {
...
// src/modules/projects/migrateBases.ts
function registerBaseMigrationCommand(ctx) {
  let running = false;
  ctx.commands.register(PROJECT_COMMANDS.migrate, () => {
    if (running) {
      new import_obsidian43.Notice("\u5B58\u91CF MOC \u6570\u636E\u5E93\u5347\u7EA7\u6B63\u5728\u8FDB\u884C\uFF0C\u8BF7\u5148\u5B8C\u6210\u5F53\u524D\u7A97\u53E3\u3002");
      return;
    }
    running = true;
    void runBaseMigration(ctx).catch((error) => {
      new MigrationReportModal(ctx.app, "\u5B58\u91CF\u6570\u636E\u5E93\u5347\u7EA7\u5931\u8D25", [errorMessage7(error)]).open();
    }).finally(() => {
      running = false;
    });
  });
...
async function runBaseMigration(ctx) {
  const scanning = new import_obsidian43.Notice("\u6B63\u5728\u68C0\u67E5\u5BFC\u822A\u4E0E\u5B58\u91CF MOC \u6570\u636E\u5E93\u2026", 0);
  let preview;
  try {
    preview = await buildMigrationPreview(ctx);
  } catch (error) {
    scanning.hide();
    new MigrationReportModal(ctx.app, "\u68C0\u67E5\u5931\u8D25", [errorMessage7(error)]).open();
    return;
  }
  scanning.hide();
  ...
  const migrating = new import_obsidian43.Notice(`\u6B63\u5728\u8FC1\u79FB ${preview.changes.length} \u4E2A\u6587\u4EF6\u2026`, 0);
  try {
...
async function buildMigrationPreview(ctx) {
  const changes = [];
  const conflicts = [];
  let unchangedCount = 0;
  const navigation = ctx.app.vault.getAbstractFileByPath(NAV_FILE);
  if (navigation instanceof import_obsidian43.TFile) {
  ...
  for (const file of ctx.app.vault.getMarkdownFiles()) {
    ...
    try {
      ...
      conflicts.push({ path: file.path, reason: `\u8BFB\u53D6\u5931\u8D25\uFF1A${errorMessage7(error)}` });
    }
  }
  return {
...
function collectResult(file, before, result, changes, conflicts) {
...
async function isMocCandidate(ctx, file) {
  var _a, _b, _c, _d, _e, _f, _g;
  if (file.path === TEMPLATE_FILES.moc) return true;
  const parentName = (_a = file.parent) == null ? void 0 : _a.name;
  if (!parentName) return false;
  const type = (_c = (_b = ctx.app.metadataCache.getFileCache(file)) == null ? void 0 : _b.frontmatter) == null ? void 0 : _c.type;
  ...
  const content = await ctx.app.vault.read(file);
  const frontmatter = (_e = (_d = content.match(/^---(?:\r\n|\r|\n)([\s\S]*?)(?:\r\n|\r|\n)---(?:\r\n|\r|\n|$)/)) == null ? void 0 : _d[1]) != null ? _e : "";
  const rawType = (_g = (_f = frontmatter.match(/^type:\s*["']?([^"'\s#]+)["']?\s*(?:#.*)?$/m)) == null ? void 0 : _f[1]) != null ? _g : "";
  return isContainerMocIdentity(file.basename, parentName, rawType);
}
async function applyMigrationsWithRollback(ctx, changes) {
  await applyMigrationBatch(
    changes,
    async (change) => {
      const currentFile = ctx.app.vault.getAbstractFileByPath(change.path);
      if (!(currentFile instanceof import_obsidian43.TFile)) {
    ...
    async (change) => {
      ...
      if (!(file instanceof import_obsidian43.TFile)) throw new Error("\u6587\u4EF6\u5DF2\u88AB\u79FB\u52A8\u6216\u5220\u9664");
      ctx.guard.mark(change.path);
...
var BaseMigrationPreviewModal = class extends import_obsidian43.Modal {
  ...
  onOpen() {
    ...
    const summary = this.contentEl.createEl("p", {
      text: `\u5F85\u5347\u7EA7 ${changes.length} \u4E2A \xB7 \u5DF2\u662F\u65B0\u7248 ${unchangedCount} \u4E2A \xB7 \u51B2\u7A81\u8DF3\u8FC7 ${conflicts.length} \u4E2A`
    });
    ...
    for (const [index, change] of changes.entries()) {
      ...
      const label = details.createEl("summary", {
        text: `${change.action} \xB7 ${change.path}`
      });
    ...
    buttonBar.style.gap = "8px";
    buttonBar.style.marginTop = "16px";
    new import_obsidian43.ButtonComponent(buttonBar).setButtonText(changes.length ? "\u53D6\u6D88" : "\u5173\u95ED").onClick(() => this.settle(false));
    if (changes.length) {
      new import_obsidian43.ButtonComponent(buttonBar).setButtonText(`\u786E\u8BA4\u8FC1\u79FB ${changes.length} \u4E2A\u6587\u4EF6`).setCta().onClick(() => this.settle(true));
    }
  }
  onClose() {
    this.settle(false);
...
var MigrationReportModal = class extends import_obsidian43.Modal {
  ...
  onOpen() {
    ...
    buttonBar.style.display = "flex";
    buttonBar.style.justifyContent = "flex-end";
    new import_obsidian43.ButtonComponent(buttonBar).setButtonText("\u5173\u95ED").setCta().onClick(() => this.close());
  }
  onClose() {
    this.contentEl.empty();
  }
};
function renderBlockDiff(before, after) {
  const beforeLines = before === null ? ["\uFF08\u5F53\u524D\u6CA1\u6709 Base\uFF09"] : before.replace(/\r\n|\r/g, "\n").split("\n");
...
async function runProjectTransition(ctx, action, onArchived) {
  try {
    ...
    if (!transition) {
      new import_obsidian44.Notice(`\u672A\u77E5\u7684\u9879\u76EE\u6D41\u8F6C\u52A8\u4F5C\uFF1A${action}`);
      return;
    }
    ...
    new import_obsidian44.Notice(`\u9879\u76EE\u72B6\u6001\u6D41\u8F6C\u5931\u8D25\uFF1A${getErrorMessage(error)}`);
  }
}
function resolveTransitionPlan(ctx, transition) {
  ...
  if (activeFolder === archiveFolder) {
    new import_obsidian44.Notice("\u9879\u76EE\u76EE\u5F55\u548C\u5F52\u6863\u76EE\u5F55\u4E0D\u80FD\u8BBE\u7F6E\u4E3A\u540C\u4E00\u8DEF\u5F84\u3002");
    return null;
  }
  ...
  if (!(mocFile instanceof import_obsidian44.TFile) || mocFile.extension !== "md") {
    new import_obsidian44.Notice("\u8BF7\u5148\u6253\u5F00\u9700\u8981\u8FDB\u884C\u72B6\u6001\u6D41\u8F6C\u7684\u9879\u76EE MOC\u3002");
    return null;
  }
  const projectFolder = mocFile.parent;
  if (!(projectFolder instanceof import_obsidian44.TFolder)) {
    new import_obsidian44.Notice("\u65E0\u6CD5\u8BC6\u522B\u5F53\u524D\u9879\u76EE\u6587\u4EF6\u5939\u3002");
    return null;
  }
  const projectName = projectFolder.name;
  const sourceProjectPath = (0, import_obsidian44.normalizePath)(`${sourceRoot}/${projectName}`);
  const expectedMocPath = resolveMocPath(app, sourceProjectPath, projectName);
  if ((0, import_obsidian44.normalizePath)(mocFile.path) !== expectedMocPath) {
    new import_obsidian44.Notice(`\u5F53\u524D\u547D\u4EE4\u53EA\u80FD\u5728\u4EE5\u4E0B\u9879\u76EE MOC \u4E2D\u6267\u884C\uFF1A${expectedMocPath}`);
    return null;
  }
  ...
  if (!MOVABLE_TYPES.includes(type)) {
    new import_obsidian44.Notice("\u5F53\u524D\u7B14\u8BB0\u4E0D\u662F\u9879\u76EE\u6216\u8BFB\u4E66\u7B14\u8BB0 MOC\uFF1A\u7F3A\u5C11 type: project\uFF08\u9879\u76EE\uFF09\u6216 type: book\uFF08\u4E66\uFF09\u3002");
    return null;
  }
  ...
  const targetProjectPath = (0, import_obsidian44.normalizePath)(`${targetRoot}/${projectName}`);
  const targetMocPath = (0, import_obsidian44.normalizePath)(`${targetProjectPath}/${mocFile.name}`);
  const existingTarget = app.vault.getAbstractFileByPath(targetProjectPath);
  if (existingTarget) {
    new import_obsidian44.Notice(`\u76EE\u6807\u4F4D\u7F6E\u5DF2\u7ECF\u5B58\u5728\u540C\u540D\u9879\u76EE\uFF0C\u64CD\u4F5C\u5DF2\u505C\u6B62\uFF1A${targetProjectPath}`);
    return null;
  }
  return {
    transition,
...
async function applyTransition(ctx, plan) {
  ...
  try {
    ...
    if (!(movedMoc instanceof import_obsidian44.TFile)) {
...
function markFolderTree(ctx, folder, targetPath) {
  ...
  import_obsidian44.Vault.recurseChildren(folder, (child) => {
    if (!(child instanceof import_obsidian44.TFile)) return;
    const relativePath = child.path.slice(sourcePath.length + 1);
    guard.mark(child.path);
    guard.mark((0, import_obsidian44.normalizePath)(`${targetPath}/${relativePath}`));
  });
}
async function reopenMovedMoc(ctx, targetMocPath) {
  const movedMoc = ctx.app.vault.getAbstractFileByPath(targetMocPath);
  if (!(movedMoc instanceof import_obsidian44.TFile)) return;
  try {
...
async function rollbackTransition(ctx, plan, original, frontmatterVisited) {
  ...
  try {
    ...
    if (sourceEntry && !(sourceEntry instanceof import_obsidian44.TFolder)) {
    ...
    if (targetEntry) {
      ...
      try {
        ...
        if (!(restored instanceof import_obsidian44.TFolder) || remains) throw renameError;
      }
    }
    if (!frontmatterVisited) return null;
    const restoredMoc = app.vault.getAbstractFileByPath(plan.expectedMocPath);
    if (!(restoredMoc instanceof import_obsidian44.TFile)) {
...
var ProjectTransitionConfirmModal = class extends import_obsidian44.Modal {
  ...
  onOpen() {
    ...
    buttonBar.style.gap = "8px";
    buttonBar.style.marginTop = "18px";
    new import_obsidian44.ButtonComponent(buttonBar).setButtonText("\u53D6\u6D88").onClick(() => this.settle(false));
    const confirmButton = new import_obsidian44.ButtonComponent(buttonBar).setButtonText(`\u786E\u8BA4${transition.label}`).setCta().onClick(() => this.settle(true));
...
function registerUpdatedMaintainer(ctx) {
  ...
  const applyUpdated = async (path) => {
    ...
    await withPreservedMarkdownScroll(
      ctx.app,
      file,
      () => ctx.app.fileManager.processFrontMatter(file, (frontmatter) => {
  ...
  ctx.app.workspace.onLayoutReady(() => {
    ctx.plugin.registerEvent(
      ctx.app.vault.on("modify", (file) => {
        ...
        if (!(file instanceof import_obsidian45.TFile) || file.extension !== "md") return;
...
async function openPeriodNote(ctx, period, options) {
  ...
  try {
    ...
    if (!title) {
      new import_obsidian46.Notice(`\u65E0\u6CD5\u4ECE\u65E5\u671F ${(_a = options == null ? void 0 : options.day) != null ? _a : ""} \u5B9A\u4F4D${period.label}`);
      return null;
    }
    const folder = periodFolderOf(ctx, period);
    const path = `${folder}/${title}.md`;
    const existing = ctx.app.vault.getAbstractFileByPath(path);
    if (existing && !(existing instanceof import_obsidian46.TFile)) {
    ...
    const message2 = error instanceof Error ? error.message : String(error);
    new import_obsidian46.Notice(`\u6253\u5F00${period.label}\u5931\u8D25\uFF1A${message2}`);
    return null;
  }
}
...
async function promptAndWriteTheme(ctx, file, period, current) {
  ...
  if (!theme) {
    new import_obsidian47.Notice(MESSAGES8.unchanged);
    return;
  }
  ctx.guard.mark(file.path);
  ...
  new import_obsidian47.Notice(`${MESSAGES8.donePrefix}${period.label}\u4E3B\u9898\uFF1A${theme}`);
}
...
function reportFailure(error) {
  const message2 = error instanceof Error ? error.message : String(error);
  new import_obsidian47.Notice(MESSAGES8.failedPrefix + message2);
}
...
var ARTWORK = {
  ...
  /** 升级存量 MOC：数据库圆柱旁一支向前箭头，表达“看清旧数据后迁到新规则” */
  [COMMAND_ICONS.migrate]: [
    "M4 6C4 4.34315 7.58172 3 12 3C16.4183 3 20 4.34315 20 6C20 7.65685 16.4183 9 12 9C7.58172 9 4 7.65685 4 6Z",
    "M4 6V11C4 12.6569 7.58172 14 12 14M4 11V16C4 17.6569 7.58172 19 12 19",
    "M15 17H21M21 17L18.5 14.5M21 17L18.5 19.5"
  ],
...
function registerZiminosIcons(plugin) {
  for (const [name, paths] of Object.entries(ARTWORK)) {
    (0, import_obsidian48.addIcon)(name, wrap(paths));
    plugin.register(() => (0, import_obsidian48.removeIcon)(name));
  }
}
function wrap(paths) {
    ...
    for (const command of this.ctx.commands.list()) {
      ...
      if (!enabled.has(id)) {
        if (existing && !existing.hasClass(HIDDEN_CLASS)) {
          ...
          if (import_obsidian49.Platform.isPhone) new import_obsidian49.Notice(MOBILE_PENDING);
        }
        continue;
      }
      if (existing) {
...
async function initializeVault(ctx, seeds) {
  try {
    ...
    if (isFirstRun && hasUserNotes(ctx, seeds)) {
      new import_obsidian50.Notice(MESSAGES9.notEmpty);
      return;
    }
    ...
    new import_obsidian50.Notice(MESSAGES9.done);
    celebrate(ctx);
    ...
    const message2 = error instanceof Error ? error.message : String(error);
    new import_obsidian50.Notice(MESSAGES9.failedPrefix + message2);
  }
}
...
var TABS = [
  ...
  },
  {
    ...
    module: "\u5DE6\u4FA7\u8FB9\u680F v1",
    status: "\u8FD0\u884C\u4E2D \xB7 \u4E09\u5341\u516D\u6761\u547D\u4EE4\u914D Pikaicons \u56FE\u6807\uFF0C\u9ED8\u8BA4\u6446\u51FA\u5341\u6761"
  }
];
var TEXTS6 = {
  initName: "\u521D\u59CB\u5316\u7B14\u8BB0\u5E93",
  ...
  filePathDesc: "\u5728\u53F3\u4E0B\u89D2\u72B6\u6001\u680F\u663E\u793A\u4F60\u6B64\u523B\u8FD9\u4E00\u7BC7\u5728\u54EA\u4E2A\u6587\u4EF6\u5939\u91CC\uFF0C\u70B9\u4E00\u4E0B\u628A\u8DEF\u5F84\u590D\u5236\u8D70\uFF08\u590D\u5236\u54EA\u4E00\u79CD\u89C1\u4E0B\u9762\u90A3\u9879\uFF09\u3002\u5173\u6389\u53EA\u662F\u6536\u8D77\u90A3\u4E00\u5757\uFF0C\u547D\u4EE4\u300C\u590D\u5236\u5F53\u524D\u7B14\u8BB0\u8DEF\u5F84\u300D\u7167\u5E38\u53EF\u7528\uFF08\u53EF\u4EE5\u53BB\u8BBE\u7F6E \u2192 \u5FEB\u6377\u952E\u7ED9\u5B83\u7ED1\u4E2A\u952E\uFF09\u3002",
  filePathScopeName: "\u590D\u5236\u54EA\u4E00\u79CD\u8DEF\u5F84",
  filePathScopeDesc: "\u5C4F\u5E55\u4E0A\u90A3\u4E00\u5757\u6C38\u8FDC\u663E\u793A\u5E93\u5185\u8DEF\u5F84\u2014\u2014\u5B83\u77ED\uFF0C\u4E14\u6BCF\u7BC7\u90FD\u4E0D\u540C\uFF1B\u8FD9\u4E00\u9879\u51B3\u5B9A\u7684\u662F\u300C\u590D\u5236\u8D70\u7684\u90A3\u4E00\u4E32\u300D\u3002\u5E93\u5185\u8DEF\u5F84\uFF0801-projects/\u5F00\u6E90\u4E4B\u9053/\u5F00\u6E90\u4E4B\u9053.md\uFF09\u8BF4\u7ED9 Obsidian \u81EA\u5DF1\u542C\uFF1A\u5199\u53CC\u94FE\u3001\u8DDF\u540C\u4E00\u4E2A\u5E93\u7684\u4EBA\u8BF4\u6211\u90A3\u7BC7\u5728\u54EA\u513F\u3002\u672C\u673A\u5B8C\u6574\u8DEF\u5F84\uFF08/\u4F60\u7684\u7528\u6237\u76EE\u5F55/\u7B14\u8BB0\u5E93/01-projects/\u5F00\u6E90\u4E4B\u9053/\u5F00\u6E90\u4E4B\u9053.md\uFF09\u8BF4\u7ED9\u5E93\u5916\u9762\u542C\uFF1A\u5728\u7EC8\u7AEF\u91CC cd \u8FC7\u53BB\u3001\u62D6\u8FDB\u522B\u7684\u7A0B\u5E8F\u3001\u4EA4\u7ED9\u4E00\u4E2A\u667A\u80FD\u4F53\u53BB\u8BFB\u90A3\u4E2A\u6587\u4EF6\u3002\u628A\u9F20\u6807\u505C\u5728\u72B6\u6001\u680F\u90A3\u4E00\u5757\u4E0A\uFF0C\u4F1A\u5148\u544A\u8BC9\u4F60\u8FD9\u4E00\u4E0B\u5C06\u590D\u5236\u4EC0\u4E48\u3002\u624B\u673A\u4E0A\u6CA1\u6709\u672C\u673A\u8DEF\u5F84\u8FD9\u56DE\u4E8B\uFF0C\u90A3\u65F6\u5B83\u81EA\u52A8\u9000\u56DE\u5E93\u5185\u8DEF\u5F84\uFF0C\u5E76\u8BF4\u4E00\u58F0\u4E3A\u4EC0\u4E48\u3002",
  recentHeading: "\u6700\u8FD1\u6587\u4EF6",
...
var SettingsPanels = class {
  constructor(host) {
     ...
     * 而重建整页会把滚动条弹回顶部——三十六行排下来，用户勾第二十行时页面一跳，
  ...
  renderInitButton(containerEl) {
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.initName).setDesc(this.describeInitState()).addButton((button) => {
  ...
  renderBooksSection(containerEl) {
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.booksHeading).setDesc(TEXTS6.booksIntro).setHeading();
    this.host.renderTextField(containerEl, BOOK_TAG_PREFIX_FIELD);
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.bookTagCountName).setDesc(TEXTS6.bookTagCountDesc).addDropdown((dropdown) => {
  ...
  renderWereadRow(containerEl) {
    ...
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.wereadName).setDesc(
      import_obsidian51.Platform.isDesktopApp ? connected ? TEXTS6.wereadConnected : TEXTS6.wereadDisconnected : TEXTS6.wereadMobile
    ).addButton((button) => {
      button.setButtonText(connected ? "\u65AD\u5F00" : "\u626B\u7801\u8FDE\u63A5").setDisabled(!import_obsidian51.Platform.isDesktopApp);
  ...
  renderInspirationPanel(containerEl) {
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.inspirationPositionName).setDesc(TEXTS6.inspirationPositionDesc).addDropdown((dropdown) => {
    ...
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.inspirationFormatName).setDesc(TEXTS6.inspirationFormatDesc).addTextArea((textArea) => {
  ...
  renderFormatSection(containerEl) {
    ...
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.formatRulesHeading).setDesc(TEXTS6.formatRulesIntro).setHeading();
  ...
  renderRuleRow(containerEl, key, name, desc) {
    new import_obsidian51.Setting(containerEl).setName(name).setDesc(desc).addToggle((toggle) => {
  ...
  renderRibbonPanel(containerEl) {
    const summary = new import_obsidian51.Setting(containerEl).setName(this.describeRibbonCount()).setDesc(TEXTS6.ribbonIntro);
  ...
  /**
   * 「已摆出 7 / 36 条」。给的是一个量级感：勾多了那条边栏会变成谁也不看的图标柱。
  ...
  renderRibbonRow(containerEl, spec) {
    ...
    const label = createFragment((frag) => {
      ...
      (0, import_obsidian51.setIcon)(iconEl, icon);
      iconEl.style.color = GROUP_COLORS[spec.group];
      frag.createSpan({ text: name });
    });
    new import_obsidian51.Setting(containerEl).setName(label).setClass("ziminos-ribbon-row").addToggle((toggle) => {
  ...
  renderExplorerPanel(containerEl) {
    ...
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.folderCountTargetName).setDesc(TEXTS6.folderCountTargetDesc).addDropdown((dropdown) => {
   ...
   */
  renderFilePathSection(containerEl) {
    this.host.renderToggle(
      containerEl,
      "showFilePath",
      TEXTS6.filePathName,
    ...
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.filePathScopeName).setDesc(TEXTS6.filePathScopeDesc).addDropdown((dropdown) => {
      for (const scope of FILE_PATH_SCOPES) {
        dropdown.addOption(scope, FILE_PATH_SCOPE_LABELS[scope]);
      }
      dropdown.setValue(this.normalizeFilePathScope(this.ctx.settings.filePathScope)).onChange(async (value) => {
  ...
  renderRecentSection(containerEl) {
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.recentHeading).setDesc(TEXTS6.recentIntro).setHeading();
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.recentLimitName).setDesc(TEXTS6.recentLimitDesc).addDropdown((dropdown) => {
    ...
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.recentSortName).setDesc(TEXTS6.recentSortDesc).addDropdown((dropdown) => {
  ...
  renderEditingPanel(containerEl) {
    ...
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.formatHeading).setDesc(TEXTS6.formatIntro).setHeading();
    this.renderFormatSection(containerEl);
  }
  /** Eagle 是编辑页的附件支线：行为/图片分流、项目归档、本机连接与设备参数收在同一段 */
  renderEaglePanel(containerEl) {
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.eagleHeading).setDesc(TEXTS6.eagleIntro).setHeading();
    ...
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.eaglePackageName).setDesc(TEXTS6.eaglePackageDesc).addButton((button) => button.setButtonText("\u663E\u793A\u5B89\u88C5\u5305").onClick(() => void this.actions.revealEaglePackage()));
    const connection = new import_obsidian51.Setting(containerEl).setName(TEXTS6.eagleStatusName).setDesc(TEXTS6.eagleStatusChecking).addButton((button) => button.setButtonText("\u914D\u5BF9").setCta().onClick(async () => {
      button.setDisabled(true);
      await this.actions.pairEagle();
      this.host.rebuild();
    })).addButton((button) => button.setButtonText("\u68C0\u6D4B").onClick(() => void this.actions.testEagle())).addButton((button) => button.setButtonText("\u65AD\u5F00").setWarning().onClick(async () => {
    ...
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.eaglePortName).setDesc(TEXTS6.eaglePortDesc).addText((text3) => text3.setPlaceholder(String(EAGLE_DEFAULTS.port)).setValue(String(this.ctx.settings.eaglePort)).onChange(async (value) => {
    ...
    new import_obsidian51.Setting(containerEl).setName(TEXTS6.eagleFolderName).setDesc(TEXTS6.eagleFolderDesc).addText((text3) => text3.setPlaceholder("\u9879\u76EE\u5916\u7559\u7A7A\uFF1A\u672A\u5F52\u7C7B").setValue(this.ctx.settings.eagleFolderId).onChange(async (value) => {
...
// src/settings.ts
var ZiminosSettingTab = class extends import_obsidian52.PluginSettingTab {
  ...
  renderTabBar(containerEl) {
    ...
    for (const tab of TABS) {
      ...
      (0, import_obsidian52.setIcon)(button.createSpan({ cls: "ziminos-settings-tab-icon" }), tab.icon);
  ...
  renderPanel(body) {
    ...
    const header2 = new import_obsidian52.Setting(body).setDesc(tab.status).setHeading();
    const title = header2.nameEl.createSpan({ cls: "ziminos-settings-page-title" });
    (0, import_obsidian52.setIcon)(title.createSpan({ cls: "ziminos-settings-page-icon" }), tab.icon);
  ...
  renderToggle(containerEl, key, name, desc, onApplied) {
    new import_obsidian52.Setting(containerEl).setName(name).setDesc(desc).addToggle((toggle) => {
  ...
  renderTextFields(containerEl, tab, advanced) {
    ...
    for (const field2 of fields) {
      ...
      if (section && section !== currentSection) {
        new import_obsidian52.Setting(containerEl).setName(section).setHeading();
      }
  ...
  renderTextField(containerEl, field2) {
    ...
    new import_obsidian52.Setting(containerEl).setName(field2.name).setDesc(desc).addText((text3) => {
...
// src/main.ts
var ZiminosPlugin = class extends import_obsidian53.Plugin {
  constructor() {
    super(...arguments);
    /**
     * 全局唯一的设置对象。
