...
package-eagle.sh - Eagle 伴侣唯一打包出口；核对两端版本后以固定白名单组装 `.eagleplugin` ZIP，产物原子落进 vault 的 ziminOS 插件目录，不安装、不读取 Eagle 用户数据
...
docs/设计规格书-V2.md - v0.4.0 起的唯一设计事实源；§26 中国农历日历、§27 审计加固、§28 Gitee 部署、§29 文件夹计数、§30 社区功能取舍、§33 片段出境口、§34 状态栏路径、§35 手机窗口口令、§36 Eagle 附件桥接（v0.22.0：稳定 itemId、双运行时、回环鉴权与 fail-closed）。与 V1 规格并存，交集处以 V2 为准
...
11. src/modules/eagle 的桌面附件边界按需使用 Electron `webUtils.getPathForFile`、`shell.showItemInFolder` 与 Node 临时文件 API —— 前者是 Electron 33+ 取得拖入 File 原路径的官方替代，后者只负责显示随包交付的 `.eagleplugin`；截图等无原路径的内存 File 才短暂落入系统临时目录，导入后立即清理。三者都在统一平台闸门确认 macOS/Windows Obsidian 桌面端之后动态 require、全程可失败，其他平台加载阶段不解析 Node/Electron。真正的资源写入不靠这些缺口：Obsidian 端走公开 `requestUrl`，Eagle 端只调用官方 `eagle.item.addFromPath/getById/open`；删掉 modules/eagle 与 eagle-companion 即让本条及本机回环出口一起消失。
</deviations>
