...
eagle-companion/ - 第一方 Eagle 后台伴侣；以官方 item/folder/app API 执行 `项目/项目名` 归档、附件导入/读取/打开，只监听本机回环，与 src/modules/eagle 共同组成一个产品的两个运行时
...
<editing>
打字时发生的四件事，它们同住「编辑」那一页——判据是同一个时刻：粘贴变成链接、附件交给 Eagle 与光标记住位置发生在你按下键的那一瞬间，排版发生在你离开这一篇之后。**粘贴成外链**：编辑器里选着字、剪贴板里是一条带协议的网址，Cmd+V 就写成 `[选中的字](网址)`；四条门槛（开关开着、这次粘贴没被别人处理过、真的选了字、真的是一条带协议的网址）全满足才动手。**Eagle 附件**（v0.22.0）：桌面端粘贴/拖入 File 时先接管默认行为，再由本机伴侣导入当前 Eagle 资源库，成功后只在 Markdown 写 `ziminos-eagle://v1/primary/{itemId}`；显示、打开都在 DOM/运行时解析，真实路径和端口不进笔记。同一资源库里移动项目或文件夹不改 `itemId`，所以链接不用更新；换库或搬动整库只需重新配对。导入失败 fail closed，明确报错且绝不偷偷在 Obsidian 本地留第二份。v0.22.1 把打开手势补到编辑器边界：阅读视图普通单击，实时预览/源码/YAML 属性值在 macOS 用 ⌘+单击、Windows 用 Ctrl+单击，单击仍留给编辑。v0.22.2 补足应用生命周期：最小化时定位后调官方 `app.show()` 恢复主窗口，完全退出时仅在用户点击之后以 `eagle://item/{itemId}` 让系统启动 Eagle 并直达附件，不存安装路径也不起后台轮询。v0.22.4 再把项目归属接进导入事务：projects 只按项目/归档根目录下的第一层文件夹认项目，main 把结果注入 Eagle；伴侣以官方 Folder API 创建或复用 `项目/项目名`，目录有重名歧义就停止，项目外附件才读取原来的固定文件夹 ID。v0.22.5 增加一个默认关闭的图片分流开关：打开后纯图片事件在 `preventDefault` 前原样放行给已有图床，PDF、压缩包、音视频等仍进 Eagle；ziminOS 不复制图床配置也不保存密钥。**光标记忆**：离开一篇笔记时记下行列与滚动位置，下次打开回到那里，重启也还在；从带锚点的双链跳进来时不插手。位置存在 cursor-positions.json，不进 data.json。**排版**（modules/format）作为这一页的后半截交付。
...
docs/设计规格书-V2.md - v0.4.0 起的唯一设计事实源；§26 中国农历日历、§27 审计加固、§28 Gitee 部署、§29 文件夹计数、§30 社区功能取舍、§33 片段出境口、§34 状态栏路径、§35 手机窗口口令、§36 Eagle 附件桥接（v0.22.0 建立稳定 itemId/双运行时/回环鉴权/fail-closed，v0.22.1–v0.22.5 依次补编辑器跳转、应用唤起、按项目自动归档与图片分流），§37 定义 v0.22.3 分栏后台写入的视口与光标契约。与 V1 规格并存，交集处以 V2 为准
