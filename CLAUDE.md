...
docs/设计规格书-V2.md - v0.4.0 起的唯一设计事实源；§26 中国农历日历、§27 审计加固、§28 Gitee 部署、§29 文件夹计数、§30 社区功能取舍、§33 片段出境口、§34 状态栏路径、§35 手机窗口口令、§36 Eagle 附件桥接（v0.22.0 建立稳定 itemId/双运行时/回环鉴权/fail-closed，v0.22.1–v0.22.4 依次补编辑器跳转、应用唤起与按项目自动归档），§37 定义 v0.22.3 分栏后台写入的视口与光标契约。与 V1 规格并存，交集处以 V2 为准
