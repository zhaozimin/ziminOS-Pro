...
transfer.ts: 附件写入边界，先按 MIME/扩展名执行图片分流，纯图片事件在 `preventDefault` 前放行；非图片接管后按宿主笔记路由导入，原路径保留文件名空白，内存 File 只短暂物化到系统临时目录。路由解析先于占位符写入，成功用稳定链接替换占位符，全部失败则恢复原选中文本。
...
render.ts: 附件呈现与窗口生命周期边界，阅读视图与实时预览通过伴侣读取内容并生成临时 blob URL，并装配 editor 扩展；只释放带 Eagle 身份的图片，同批 DOM 变更先统一释放再水合，保证节点移动后可复用且不清空普通图片。
