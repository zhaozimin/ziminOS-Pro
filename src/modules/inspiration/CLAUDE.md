...
capture.ts: 唯一副作用编排器。注册 capture-inspiration 命令，用 core/modals 收集一行输入，经设置解析出安全目标路径；目标不存在则逐级建目录并播种带 Dataview 查询的笔记，存在则用 Vault.process 原子更新。灵感集出生时经全局 SelfWriteGuard 标记；往已有灵感集里插入的那一条是用户记的，不标记，updated 照记。错误只转成中文 Notice。
