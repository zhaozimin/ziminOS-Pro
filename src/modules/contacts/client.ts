 ...
 *          依赖 ./identity 的 liveNotesOfType/descriptionOf/askDescription、./moc 的新旧 MOC 寻址、
 *          ./templates 的生成器与旧档案视图补齐器 ensureClientViews
 ...
 *        「补齐客户档案检索」给旧档案补上后来才进模板的视图（v0.23.0 的客户答疑、v0.38.0 的相关项目），
 *        补什么、补在哪由 templates 的 CLIENT_SECTIONS 一张表回答，本文件只负责找出客户档案并写回。
 *        「新建客户」与「新建人脉」守同一套建档次序：问完称呼立刻查重（同名直接打开，后面几问
 *        对它毫无意义），一句话简介必填且放最后，它与人脉用的是同一个问法（v0.38.0）。
...
import { askDescription, descriptionOf, liveNotesOfType } from './identity';
...
/**
 * 旧档案不会因插件升级被静默改写；用户明确运行一次命令，才给所有客户补上后来才进模板的只读视图
 * （相关项目、客户答疑）。新模板天然已经有它们，所以重复运行只会报告 0，不制造第二份标题或代码块。
 */
async function backfillClientViews(ctx: ZiminosContext): Promise<void> {
    try {
        let updated = 0;
...
async function createClient(ctx: ZiminosContext): Promise<void> {
    try {
        ...
        // 同名直接打开而不覆盖，并且查在问渠道之前：打开旧档案时后面几问一个都用不上
