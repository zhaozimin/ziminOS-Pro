 ...
 *           后台写入的分栏滚动保护与它兑现不了的那一种（正开着的那一篇根本不写）、
 ...
 * 用户正开着的那一篇，绝不写盘。
 *
 * 这条回归是真机报上来的：在一篇满是远端图片的笔记里打字，每隔几秒 Obsidian 就弹
 * 「已被外部修改，正在自动合并更改」，然后视口跳到同一张图上。
 * 因为 modify 事件来自 Obsidian 自己的保存而不是用户停手，两秒后那一笔 updated
 * 正好落在一个还有未保存改动的编辑器上，合并会把整个文档重新灌回去。
 * 所以这里不 grep 源码，而是把事件按真机的顺序喂进去，直接验「有没有写盘」。
 */
test('updated 记账绝不落在用户正开着的那一篇上，等他走开才补', async () => {
    const { registerUpdatedMaintainer } = await loadTypeScript(
        'src/modules/projects/updatedMaintainer.ts',
        {
            obsidianStub: [
                "import moment from 'moment';",
                'export { moment };',
                // 测试替身只需回答「这是不是一个文件」，故直接给 instanceof 换一套判据
                'export class TFile {',
                '    static [Symbol.hasInstance](value) {',
                ...
                '    }',
                '}',
                // 分栏里那个 view 要能被 instanceof 认出来，否则「还开着没有」问不出结果
                'export class MarkdownView {',
                '    static [Symbol.hasInstance](value) {',
                "        return typeof value === 'object' && value !== null",
                "            && value.kind === 'markdown';",
    ...
    try {
        const files = {
            ...
            'C.md': { path: 'C.md', extension: 'md' },
        };

        /** 写进了哪几篇，按顺序 */
        const writes = [];
        const handlers = { modify: [], 'active-leaf-change': [], 'file-open': [] };
        let activePath = 'A.md';
        /** 此刻开着哪几个标签页。关掉最后一个之后它是空的，而活动文件仍报最近那一篇 */
        let openTabs = ['A.md'];

        const ctx = {
            settings: { autoUpdated: true, dateTimeFormat: 'YYYY-MM-DD HH:mm:ss' },
            guard: { mark: () => {}, isRecent: () => false },
            ...
            app: {
                ...
                workspace: {
                    ...
                    getActiveFile: () => files[activePath] ?? null,
                    iterateAllLeaves: (visit) => {
                        // 替身要撑得起两个消费方：isDisplayed 只问 file.path，
                        // 而 withPreservedMarkdownScroll 会接着问模式与滚动位置
                        for (const path of openTabs) {
                            visit({
                                view: {
                                    ...
                                    getMode: () => 'source',
                                    currentMode: { getScroll: () => 120, applyScroll: () => {} },
                                },
                            });
                        }
                    },
                ...
                metadataCache: { getFileCache: () => ({ frontmatter: { updated: 'old' } }) },
                fileManager: {
        ...
        const fireModify = (path) => {
            for (const handler of handlers.modify) handler(files[path]);
        };
        const fireLeafChange = () => {
            for (const handler of handlers['active-leaf-change']) handler();
        };
        ...
        assert.deepEqual(writes, [], '正开着的那一篇被写盘了，真机上这就是那个合并弹窗');

        // 2. 没开着的那一篇照旧走防抖，背景写入这条路不能被这次修复顺手掐掉
        fireModify('C.md');
        await flushTimers();

        assert.deepEqual(writes, ['C.md']);

        // 3. 点右侧栏的日历这类非 FileView：活动文件没变，不算走开，也不该落盘
        fireLeafChange();
        await flushTimers();

        assert.deepEqual(writes, ['C.md'], '点侧边栏被误当成走开了');
        ...
        assert.deepEqual(writes, ['C.md', 'A.md']);

        // 5. 再切一次不该凭空多一笔——债已经还过了
        activePath = 'A.md';
        fireLeafChange();
        await flushTimers();
        ...
        /*
         * 6. 关掉这篇笔记也算走开，而这一格差点漏掉。
         *
         * 关掉最后一个标签页时 active-leaf-change 照样触发，可 getActiveFile() 仍报刚关掉
         * 的那一篇——它是「最近活动过的文件」（官方语义）。只问活动文件的话，这一笔会留在
         * dirtyWhileOpen 里等到下一次开笔记才补，中间退出 Obsidian 就丢了。
         */
        fireModify('A.md');
        await flushTimers();

        assert.deepEqual(writes, ['C.md', 'A.md'], '打字期间又写了一次');
